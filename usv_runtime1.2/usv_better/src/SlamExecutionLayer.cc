...
cv::Mat buildRedMaskFromBgr(const std::uint8_t* bgr_bytes,
    ...
    cv::Mat mask;
    const cv::Scalar lower_red(0, 186, 71);
    const cv::Scalar upper_red(179, 255, 255);
        ...
        try {
            ...
            for (const int row_index : row_indices) {
                for (int x = 0; x < sample_limit; x += stride) {
                    ...
                    const std::uint8_t r_mask = red_mask.at<std::uint8_t>(row_index, x);
                    out->rgb_row.push_back(r_mask);
